...
if __name__ == '__main__':
    ...
    if mode == 'zoom' and 'zoom' in form:
        ...
        dim = '{} {}'.format(
            form.getfirst('width', '0') if mode != 'height' else 0,
            form.getfirst('height', '0') if mode != 'width' else 0)
    ...
    if cfg.SCALE and 'scale' in form:
        ...
        if re.match('^[0-9.-]+, *[0-9.-]+$', scalepos):
            options['add-layers'] = ','.join(
                [cfg.ROUTE, cfg.SCALE]) if 'add-layers' in options else cfg.SCALE
    ...
    style = form.getfirst('style') if form.getfirst(
        'style', '') in cfg.STYLES else next(iter(cfg.STYLES))
    fmt = form.getfirst('format') if form.getfirst(
        'format', '') in cfg.FORMATS else next(iter(cfg.FORMATS))
    options['format'] = fmt
    ...
    if code == 0 and os.stat(outputName).st_size > 0:
        ...
        print 'Content-Disposition: attachment; filename={}-{}.{}'.format(
